...
// TestAgent plugin content (TypeScript), embedded from the source file
const TESTAGENT_PLUGIN_CONTENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/agent-support/testagent/git-ai.ts"
));

pub struct TestAgentInstaller;

impl TestAgentInstaller {
    ...
    fn generate_plugin_content(binary_path: &Path) -> String {
        ...
        TESTAGENT_PLUGIN_CONTENT.replace("__GIT_AI_BINARY_PATH__", &path_str)
    }
}

impl HookInstaller for TestAgentInstaller {
    fn name(&self) -> &str {
        "TestAgent"
    }
    ...
    fn check_hooks(&self, params: &HookInstallerParams) -> Result<HookCheckResult, GitAiError> {
        let has_binary = binary_exists("testagent");
        let has_global_config = home_dir().join(".config").join("testagent").exists();
        let has_local_config = Path::new(".testagent").exists();
...
mod tests {
    ...
    #[test]
    fn test_testagent_install_plugin_creates_file_from_scratch() {
        ...
        let generated = TestAgentInstaller::generate_plugin_content(&binary_path);
        ...
        // Uses the testagent preset with session_id-based hook input and absolute path
    ...
    #[test]
    fn test_testagent_plugin_content_is_valid_typescript() {
        let content = TESTAGENT_PLUGIN_CONTENT;
    ...
    #[test]
    fn test_testagent_plugin_placeholder_substitution() {
        let binary_path = create_test_binary_path();
        let content = TestAgentInstaller::generate_plugin_content(&binary_path);
        ...
        assert!(content.contains("${GIT_AI_BIN} checkpoint testagent"));
    }

    #[test]
    fn test_testagent_plugin_skips_if_already_exists() {
        let (_temp_dir, plugin_path) = setup_test_env();
        let binary_path = create_test_binary_path();
    ...
    #[test]
    fn test_testagent_plugin_updates_outdated_content() {
        let (_temp_dir, plugin_path) = setup_test_env();
        let binary_path = create_test_binary_path();
    ...
    #[test]
    fn test_testagent_plugin_windows_path_escaping() {
    ...
    #[test]
    fn test_testagent_plugin_handles_empty_directory() {
        let temp_dir = TempDir::new().unwrap();
        let binary_path = create_test_binary_path();
        let plugin_path = temp_dir
            .path()
