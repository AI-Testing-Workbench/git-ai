...
if ([string]::IsNullOrWhiteSpace($env:GIT_AI_EXTENSION_PATH)) {
    Write-Host "Installing git-ai from VSCode extension bundle..."
} else {
    Write-Host "Installing git-ai from VSCode extension bundle..."
}
...
# Priority: 1. Local binary override, 2. VSCode extension bundle (forced when set), 3. Download from GitHub
$downloadedBinaryName = $null
# Debug output - show all env vars starting with GIT_AI
Write-Host "=== DEBUG: Environment Variables ==="
Get-ChildItem Env: | Where-Object { $_.Name -like "GIT_AI*" } | ForEach-Object {
    Write-Host "  $($_.Name) = '$($_.Value)'"
}
Write-Host "=== END DEBUG ==="

# Fallback: if GIT_AI_EXTENSION_PATH not set, infer from script location
# Script is at: extension/scripts/install.ps1
# Resources are at: extension/resources/
if ([string]::IsNullOrWhiteSpace($env:GIT_AI_EXTENSION_PATH)) {
    # Try multiple methods to get script directory
    $scriptPath = $MyInvocation.MyCommand.Path
    if ([string]::IsNullOrWhiteSpace($scriptPath) -or $scriptPath -eq "") {
        $scriptPath = $PSCommandPath
    }
    Write-Host "=== DEBUG: Script path via MyInvocation = '$scriptPath' ==="
    Write-Host "=== DEBUG: Script path via PSCommandPath = '$PSCommandPath' ==="
    Write-Host "=== DEBUG: PSScriptRoot = '$PSScriptRoot' ==="
    
    if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) {
        # PSScriptRoot is available in PS 3.0+
        # Go up from scripts/ to extension/ root, then to resources/
        $extensionRoot = Split-Path -Parent $PSScriptRoot
        $env:GIT_AI_EXTENSION_PATH = Join-Path $extensionRoot "resources"
    } elseif (-not [string]::IsNullOrWhiteSpace($scriptPath)) {
        $scriptDir = Split-Path -Parent $scriptPath
        $extensionRoot = Split-Path -Parent $scriptDir
        $env:GIT_AI_EXTENSION_PATH = Join-Path $extensionRoot "resources"
    } else {
        Write-ErrorAndExit "Cannot determine script location. Please set GIT_AI_EXTENSION_PATH environment variable."
    }
    Write-Host "=== DEBUG: Inferred extension root = '$extensionRoot' ==="
    Write-Host "=== DEBUG: Inferred extension path = '$($env:GIT_AI_EXTENSION_PATH)' ==="
}
...
if (-not [string]::IsNullOrWhiteSpace($env:GIT_AI_LOCAL_BINARY)) {
...
} elseif (-not [string]::IsNullOrWhiteSpace($env:GIT_AI_EXTENSION_PATH)) {
    # VSCode extension bundle - FORCED, no download fallback
    # The extension MUST contain the binary; this is required for VSIX installation.
    $extensionPath = $env:GIT_AI_EXTENSION_PATH
    if (-not (Test-Path -LiteralPath $extensionPath)) {
        Remove-Item -Force -ErrorAction SilentlyContinue $tmpFile
        Write-ErrorAndExit "Extension resources path not found: $extensionPath"
    }

    $candidateNames = @(
        (Join-Path $extensionPath "$binaryName.exe"),
        (Join-Path $extensionPath "$binaryName"),
        (Join-Path $extensionPath 'git-ai.exe'),
        (Join-Path $extensionPath 'git-ai'),
        (Join-Path (Join-Path $extensionPath 'bin') "$binaryName.exe"),
        (Join-Path (Join-Path $extensionPath 'bin') "$binaryName"),
        (Join-Path (Join-Path $extensionPath 'bin') 'git-ai.exe'),
        (Join-Path (Join-Path $extensionPath 'bin') 'git-ai'),
        (Join-Path (Join-Path $extensionPath 'resources') "$binaryName.exe"),
        (Join-Path (Join-Path $extensionPath 'resources') "$binaryName")
    )
    ...
    if (-not $sourceBinary) {
        Remove-Item -Force -ErrorAction SilentlyContinue $tmpFile
        Write-ErrorAndExit "Could not find git-ai binary inside extension resources at: $extensionPath"
    }

    Write-Host "Installing git-ai from VSCode extension: $sourceBinary"
    Copy-Item -Force -Path $sourceBinary -Destination $tmpFile

    $sourceFileName = Split-Path $sourceBinary -Leaf
    if ($sourceFileName -match '\.exe$') {
        $downloadedBinaryName = $sourceFileName
    } else {
        $downloadedBinaryName = $binaryName
    }
} else {
    # VSCode extension installation: MUST use bundled binary, no download fallback
    Remove-Item -Force -ErrorAction SilentlyContinue $tmpFile
    Write-ErrorAndExit "git-ai binary not found in extension bundle. Please reinstall the VSCode extension."
}
...
# Skip checksum verification in VSCode extension context
